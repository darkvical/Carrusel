...
public class GwtFancyLinkImage extends Widget {
    private final List<ImageElement> images = new ArrayList<ImageElement>();
	//private final List<AnchorElement> anchors = new ArrayList<AnchorElement>();
    ...
    private Map<String, String> mpLinks;

    public GwtFancyLinkImage() {
        ...
        getElement().appendChild(anchorElement);
        
        anchorElement.setTarget("_blank");
        
        mpLinks = new HashMap<String, String>();
    }
    ...
    private Timer getAutoBrowseTimer() {
        if (autoBrowseTimer == null) {
            autoBrowseTimer = new Timer() {
                ...
                @Override
                public void run() {
                    if (GwtFancyLinkImage.this.isVisible()) {
                        showNextImage();
                    }
                }
    ...
    public void setImage(String urlImage, String url, int index) {
        if (index >= images.size()) {
            addImage(urlImage, url);
        } else {
            ImageElement replaced = getImageElement(index);
    ...
    private ImageElement getImageElement(int index) {
    	return images.get(index);
    }

    public int addImage(String urlImage, String url) {
        ImageElement image = createImageElement(urlImage);
        images.add(image);
        mpLinks.put(urlImage, url);
    ...
    public void trimImages(int amount) {
        while (images.size() > amount) {
            removeImage(getImageElement(amount).getSrc());
        }
    ...
    public void showImage(String url) {  
        ...
        for (int i = 0; i < images.size(); ++i) {
            if (getImageElement(i).getSrc().endsWith(url)) {
                showImage(i);
                break;
            }
        }
    ...
    public void showImage(int index) {
        ...
        if (index != currentIndex) {
            ...
            if(mpLinks.get(currentImage.getSrc()).equals(FancyLinkImage.URL_VOID)){
            	anchorElement.setTarget("");
            }else{
            	anchorElement.setTarget("_blank");
            }
            
            anchorElement.setHref(mpLinks.get(currentImage.getSrc()));
